(function($){
	...
	var PageSwitch = (function() {
		...
		PageSwitch.prototype = {
			...
			init : function () {
				...
				me.cam_sections = me.element.find(me.selectors.cam_sections);
				me.cam_section = me.element.find(me.selectors.cam_section);
			...
			getPagesCount : function () {
				return this.cam_section.length;
			},
			...
			_initLayout : function () {
				...
				var width = (me.pagesCount * 100) + "%",
				 ...
				 me.cam_sections.width(width);
				 me.cam_section.width(cellWidth).css("float","left");
			},
			...
			_initEvent : function () {
				...
				$(window).resize(function() {
					var currentLength = me.switchLength(),
							 offset = me.settings.direction ? me.cam_section.eq(me.index).offset().top : me.cam_section.eq(me.index).offset().left;
				...
				// 绑定transitionend事件
				me.cam_sections.on("transitionend webkitTransitionEnd oTransitionEnd otransitionend", function() {
					// 解锁
					me.canScroll = true;
			...
			_scrollPage : function () {
				var me = this,
						 dest = me.cam_section.eq(me.index).position(); // 相对父元素的偏移
				if (!dest) return;
				...
				// 支持transition的浏览器
				if (_prefix) {
					me.cam_sections.css(_prefix+"transition","all "+me.settings.duration+"ms "+me.settings.easing);
					var translate = me.direction ? "translateY(-"+dest.top+"px)" : "translateX(-"+dest.left+"px)";
					me.cam_sections.css(_prefix+"transform",translate);
				}
				// 不支持transition的浏览器
				else {
					var animateCss = me.direction ? {top:-dest.top} : {left: -dest.left};
					me.cam_sections.animate(animateCss, me.settings.duration, function() {
						// 解锁
						me.canScroll = true;
						if (me.settings.callback && $.type(me.settings.callback) === 'function') {
							me.settings.callback();
	...
	$.fn.PageSwitch.defaults = {
		selectors : {
			cam_sections : ".cam_sections",  // 容器
			cam_section : ".section", // 每一页
			pages : ".pages", // 小圆点
			active : ".active" // 当前激活页
		},
		index : 0, // 开始位置
